...
public:
     ...
     * <remarks>Casey Ugone, 5/20/2013.</remarks>
     * <param name="dimensions">The half-width and half-height.</param>
     **************************************************************************************************/
    void SetExtents(const a2de::Vector2D& half_extents);
...
Trigger<T>::Trigger() : _area(nullptr), _previous_objects(), _current_objects(), _temp_objects() {
    ...
    _area->SetBoundingRectangle(new a2de::AABB(a2de::Transform(), a2de::Vector2D(), a2de::Color::YELLOW()));
}
...
Trigger<T>& Trigger<T>::operator=(const Trigger<T>& rhs) {
    ...
    IBoundingBox* rbb = rhs.GetBody()->GetBoundingRectangle();
    if(rbb && dynamic_cast<AABB*>(rbb)) {
        bb = new AABB(*dynamic_cast<AABB*>(rbb));
    }
    if(rbb && dynamic_cast<OBB*>(rbb)) {
        bb = new OBB(*dynamic_cast<OBB*>(rbb));
    }
    this->GetBody()->SetBoundingRectangle(bb);
...
void Trigger<T>::Update(double /*deltaTime*/) {
    ...
    std::for_each(just_entered.begin(), just_entered.end(), [this](T elem)
    {
        this->OnEnter(elem);
    });

    std::for_each(still_in.begin(), still_in.end(), [this](T elem)
    {
        this->OnTick(elem);
    });

    std::for_each(just_exited.begin(), just_exited.end(), [this](T elem)
    {
        this->OnExit(elem);
    });
...
template<class T>
void Trigger<T>::SetExtents(const a2de::Vector2D& half_extents) {
    _area->SetBoundingRectangle(new a2de::AABB(this->GetPosition(), half_extents, a2de::Color::YELLOW()));
}

template<class T>
const a2de::Vector2D& Trigger<T>::GetExtents() const {
    return _area->GetBoundingRectangle()->GetHalfExtents();
}

template<class T>
a2de::Vector2D& Trigger<T>::GetExtents() {
    return const_cast<a2de::Vector2D&>(static_cast<const Trigger<T>&>(*this).GetExtents());
}

template<class T>
void Trigger<T>::OnEnter(T entered_object) {
    RegisterObject(entered_object);
}

template<class T>
void Trigger<T>::OnExit(T exited_object) {
    UnregisterObject(exited_object);
}

...
A2DE_BEGIN
    ...
    Sprite* Sprite::CreateSprite(const std::string& name, BITMAP* file) {
        if(name.empty()) {
            throw Exception("Unnamed sprites not allowed.");
        }
        if(file == nullptr) {
            throw Exception("BITMAP pointer must point to valid BITMAP.");
        }
...
Sprite* Sprite::CreateSprite(const std::string& file) {
    if(file.empty() == false) {
        if(exists(file.c_str()) == false) {
            std::string fname(get_filename(file.c_str()));
            if(fname.find('#', 0) == -1) {
                throw FileNotFoundException(file);
            } else {
                throw Exception("PACKFILES not yet supported.");
            }
        }
    } else {
        throw Exception("Use CreateSprite(std::string name, BITMAP* file) named constructor for creating Sprites from existing BITMAPs");
    }
...
Sprite::Sprite(const std::string& name, BITMAP* file) throw(...)
    : _file(name),
    _dimensions(0.0, 0.0),
    _image(nullptr),
    _position(0.0, 0.0),
    ...
    _axis(a2de::SpriteHandler::AXIS_NONE) {
        ...
        if(_image != nullptr) {
            _dimensions = Vector2D(_image->w, _image->h);
            _frameDimensions = _dimensions;
...
Sprite::Sprite(const std::string& file)
    ...
    _tintIntensity(0),
    _alpha(255),
    _alphaChannelPresent(std::string(get_extension(_file.c_str())) == "tga"),
    _axis(a2de::SpriteHandler::AXIS_NONE) {
        if(_image != nullptr) {
            _dimensions = Vector2D(_image->w, _image->h);
            _frameDimensions = _dimensions;
...
Sprite::Sprite(const Sprite& sprite)
    ...
    _tint(sprite._tint),
    _tintIntensity(sprite._tintIntensity),
    _alpha(sprite._alpha),
    _alphaChannelPresent(sprite._alphaChannelPresent),
    _axis(sprite._axis) {
        if(_image != nullptr) {
            _dimensions = Vector2D(_image->w, _image->h);
            _frameDimensions = _dimensions;
...
Color& Sprite::GetTint() {
    return const_cast<Color&>(static_cast<const Sprite&>(*this).GetTint());
}

unsigned char Sprite::GetTintIntensity() const {
    return _tintIntensity;
}
unsigned char Sprite::GetTintIntensity() {
    return static_cast<const Sprite&>(*this).GetTintIntensity();
}
...
fixed Sprite::GetScaleAsFixed() const {
    return (ftofix( (_scaleDimensions.GetX() <= _scaleDimensions.GetY() ? _scaleDimensions.GetX() : _scaleDimensions.GetY()) ) );
}

fixed Sprite::GetScaleAsFixed() {
    return static_cast<const Sprite&>(*this).GetScaleAsFixed();
}
...
void Sprite::SetTintIntensity(unsigned char intensity) {
    _tintIntensity = intensity;
}
...
Sprite& Sprite::operator=(const Sprite& rhs) {
    ...
    this->_dimensions = Vector2D(this->_image->w, this->_image->h);
    ...
    this->_tintIntensity = rhs._tintIntensity;
    this->_alpha = rhs._alpha;
    this->_alphaChannelPresent = rhs._alphaChannelPresent;
    this->_axis = rhs._axis;
    return *this;
}


bool Sprite::HasAlphaChannel() const {
    return _alphaChannelPresent;
}

bool Sprite::HasAlphaChannel() {
    return static_cast<const Sprite&>(*this).HasAlphaChannel();
}
...
void Sprite::Animate(const std::string& /*name*/, const AnimationHandler::DIRECTION& /*dir*/, double /*deltaTime*/) {
    return;
}
...
BITMAP* Sprite::GetSheet() const {
    return Sprite::GetImage();
}
BITMAP* Sprite::GetSheet() {
...
void Sprite::ApplyAlphaChannel(bool has_alpha) {
    _alphaChannelPresent = has_alpha;
}
